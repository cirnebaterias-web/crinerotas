...
test('upgrades an existing browser database from v1 to v7 without deleting durable records', async ({ page }) => {
  await page.goto('/');
  ...
  expect(migrated).toEqual({ logicalVersion: 7, hasSessions: true, counts: [1, 1, 1] });
});
