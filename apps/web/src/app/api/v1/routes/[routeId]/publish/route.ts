...
import { publishRouteRequestSchema, routePublishPath } from '@cirne/contracts';
import { publishRoute } from '@/server/routes/service';
...
export async function POST(
  ...
  try {
    ...
    const parsed = publishRouteRequestSchema.safeParse(await readRouteJson(request));
    if (!parsed.success) {
      throw new RouteRequestError(422, 'VALIDATION_FAILED', 'Rota inválida.', false);
    }
    const { routeId } = await context.params;
    const result = await publishRoute(routeId, parsed.data, new SupabaseRouteRepository(client));
    ...
    logRouteRequest({
      requestId,
      routeTemplate: routePublishPath('{routeId}'),
      method: 'POST',
      status,
      startedAt,
    });
