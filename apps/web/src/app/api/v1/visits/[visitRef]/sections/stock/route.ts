...
import { saveStockRequestSchema, visitStockPath } from '@cirne/contracts';
...
import { publicVisitMessage, saveVisitStock } from '@/server/visits/service';
...
export async function PUT(
  ...
  try {
    ...
    const parsed = saveStockRequestSchema.safeParse(await readVisitJson(request));
    if (!parsed.success) {
      throw new VisitRequestError(422, 'VALIDATION_FAILED', publicVisitMessage('VALIDATION_FAILED'), false);
    }
    const result = await saveVisitStock(
      visitRef,
      deviceId,
      idempotencyKey,
      parsed.data,
    ...
    try {
      ...
      logRequest(createLogger(config.LOG_LEVEL), {
        requestId,
        routeTemplate: visitStockPath('{visitRef}'),
        method: 'PUT',
        status,
