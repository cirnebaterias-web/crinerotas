...
select is((select count(*)::integer from api.role_permissions), 4, 'only minimal identity capabilities are seeded');
...
select is(api.get_my_identity() -> 'capabilities', '["identity.read_self"]'::jsonb, 'seller A receives minimal capability');
